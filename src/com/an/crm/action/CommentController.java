...
public class CommentController {
    ...
    public Comment load(@PathVariable(value="id") Integer id)
            throws BadRequestException {
    ...
    public Comment insert(@RequestBody @Valid Comment comment, Errors rst)
            throws BadRequestException {
        if (rst.hasErrors()) {
    ...
    public Comment update(@RequestBody @Valid Comment comment, Errors rst) throws BadRequestException {
        if (rst.hasErrors()) {
            throw new BadRequestException(rst);
        } else if (commentDao.update(comment) != 1) {
            throw new BadRequestException("保存失败！");
        } else {
            return comment;
        }
